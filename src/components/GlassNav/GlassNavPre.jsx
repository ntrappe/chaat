/**
 * @fileoverview GlassNavPre component file. Renders a component that displays
 * or hides a sidenav toggle. Based on 
...
const NavPreWrapper = styled.div`
  ...
  .sidenav-toggle {
    ...
    align-self: center;
    margin: 0 -5px;
    color: ${(props) => (props.color === 'dark' ? `var(--fg-dark-icon)` : `var(--fg-light-icon)`)};
  }
  ...
  .sidenav-separator {
    height: .8em;
    width: 1px;
    background: ${(props) => (props.color === 'dark' ? `var(--fg-dark-icon)` : `var(--fg-light-icon)`)};
    align-self: center;
    margin: 0 1.29rem;
  }
`;
...
/**
 * GlassNavPre component.
 * Renders a responsive sidenav toggle.
 *
 * @function GlassNavPre
 * @param {boolean} props.sidenav - Whether to display a sidenav toggle or not
...
function GlassNavPre({ sidenav=true, color }) {
   ...
   */
  const triggerSidebarToggle = () => window.dispatchEvent(new Event('toggle sidebar'));

  return (
    <NavPreWrapper className='nav-pre-title' color={color}>
      {sidenav && (
        <div className='sidenav-toggle-wrapper' color={color}>
          <button 
            ...
            color={color}
          >
            <span className='sidenav-icon' color={color}>
              <svg 
                className='menu-icon'
          ...
          <span className='sidenav-separator' color={color}></span>
        </div>
      )}
    </NavPreWrapper>
  )
